...
import textermod.CharTools;

using StringTools;

// Made by ShaharMS! I just modified the code a little bit as it was broken.
...
{
	public var rtlCharacters:String = CharTools.allRtlChars.replace(" ", "");
	...
	**/
	public function new(X:Float = 0, Y:Float = 0, Width:Int = 150, height:Null<Int> = null, ?Text:String, size:Int = 8,
	...
	**/
	function specialKeysDown(key:KeyCode, modifier:KeyModifier)
	{
		...
		else if (key == KeyCode.DELETE)
		{
			...
			{
				if ((text.charAt(caretIndex + 1) != "" && CharTools.isRTL(text.charAt(caretIndex + 1)))
					|| CharTools.isRTL(text.charAt(caretIndex)))
				{
					trace((text.charAt(caretIndex + 1) != "" && CharTools.isRTL(text.charAt(caretIndex + 1))));
					trace(CharTools.isRTL(text.charAt(caretIndex)));
